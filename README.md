...
本系統的核心是 **Graph Editor (圖形編輯器)**，它將對話流程轉化為類似「演出時間軸」的概念。每一個節點都是一個獨立、具體的動作（如顯示文字、角色登場、播放動畫），讓您可以精準地控制演出的每一個細節。

---

## 🔗 快速連結 (Quick Links)

* **詳細使用手冊 / 文件**：[`Packages/com.sg.dialogue/README.md`](Packages/com.sg.dialogue/README.md)
...
* **Unity**：2022.3+（URP/HDRP/內建管線依您的專案設定而定）
* **安裝方式**：Unity Package Manager（Add package from git URL...）

> 備註：Spine / Live2D 整合為「可選功能」。若您的專案不需要相關功能，不必安裝對應套件。

---

## ✨ 核心功能 (Core Features)

*   **視覺化節點編輯器**: 提供直觀的圖形介面，透過拖拉和連線即可編排複雜的對話流與事件序列。
*   **時間軸式流程**: 將對話視為一場演出。每個節點都是時間軸上的一個動作，讓您能精準控制節奏。
*   **高度可擴充**: 採用單一職責原則，新增功能（如新的角色動作、鏡頭效果）就像建立一個新的節點類型一樣簡單。
*   **動態變數系統**:
    *   支援**全域變數** (跨場景保存) 和**局部變數** (僅限當前對話)。
    *   獨特的**資料提供者 (Data Provider)** 模式，可以從任何遊戲邏輯（如玩家等級、金錢）動態讀取數值並顯示在對話中，實現完美解耦。
...
請打開 Unity 的 `Window > Package Manager`，點右上角 `+`，選擇 **Add package from git URL...**，並依序加入以下 URL。
...
> 若您需要整合 Live2D，請參考 package 內文件的「第三方整合」章節（連結見上方 Quick Links）。

### 2) 安裝對話系統 (Install Dialogue System)

在所有相依套件都安裝完畢後，使用相同方式加入本系統的 URL：
...
更完整的使用手冊、節點參考和進階教學都包含在 package 本身之中，請見：
